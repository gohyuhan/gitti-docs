---
sidebar_position: 4
description: Interactively stage and unstage files in Gitti, including line-level staging, unstaging and discard, with a syntax-aware diff viewer.
keywords:
...
:::info
During operations like Merge, Rebase, or Cherry-Pick, use the global key **`C`** to **Continue** or **`ctrl+a`** to **Abort** the operation after resolving conflicts.
:::
...
For precise control, you can stage, unstage, or discard specific lines within a file.

1.  **Enter Mode**: While viewing a file in the **Detail View** (Right Panel) through the **Modified Files** panel, press `L`.
2.  **Navigate**: Use `↑` and `↓` arrow keys to move line by line.
3.  **Toggle Stage/Unstage**: Press `space` to stage or unstage the currently selected line.
4.  **Discard Line**: Press `d` to discard the selected line.
    - **Note**: If you discard a line that is currently staged, it will be unstaged (reverted to the staging area state), effectively undoing the stage.
5.  **Exit**: Press `esc` to exit line editing mode.
