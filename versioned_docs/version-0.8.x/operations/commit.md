---
sidebar_position: 5
description: Write, amend and sign commits in Gitti with a dedicated commit UI, GPG/SSH commit signing and a visual commit log and branch graph.
keywords:
  - git commit tui
  - amend commit
...
1.  **Navigate**: Navigate to the Commit Log component (press `3`).
2.  **Select**: Select the commit to tag using `↑/↓`.
3.  **Trigger**: Press `t` to open the tag menu.
4.  **Input Name**: Enter the tag name.
...
1.  **Navigate**: Navigate to the Commit Log component (press `3`).
2.  **Select**: Select the commit to revert using `↑/↓`.
3.  **Trigger**: Press `ctrl+r`.
...
Rewrite individual commits in history — drop, reword, or squash them. See the dedicated [Interactive Rebase](./interactive-rebase.md) page for full details.

1.  **Navigate**: Press `3` to focus the **Commit Log** panel.
2.  **Select**: Highlight the target commit using `↑/↓`.
3.  **Trigger**: Press `i` to open the operation selector.
4.  **Choose**: Select **Drop**, **Reword**, **Fixup**, or **Squash**.
5.  **Complete**: Follow the prompts for the selected operation.
