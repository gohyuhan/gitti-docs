---
sidebar_position: 5
description: Configure Gitti — language, default init branch, editor, commit log limit, commit graph, merge strategy, reflog limit, logging and signing UI behavior.
keywords:
  - gitti config
  - gitti configuration
  - gitti editor
...
You can configure the editor that Gitti will use when opening files for editing (e.g., when pressing `e` on a file) or when opening the current repository (pressing `O`). The default editor is `vim`.
