---
sidebar_position: 4
description: Gitti でファイルをインタラクティブにステージ／アンステージ。行単位のステージング・アンステージ・破棄、構文対応の差分ビューアに対応。
keywords:
  - git インタラクティブステージング
  - 行ステージング git
  - git 差分ビューア
...
4.  **行の破棄**: `d` を押して、選択した行を破棄します。
    - **注意**: 現在ステージングされている行を破棄する場合、その行はアンステージング（ステージングエリアの状態に戻る）され、実質的にステージングが取り消されます。
5.  **終了**: `esc` を押して行ステージングモードを終了します。
