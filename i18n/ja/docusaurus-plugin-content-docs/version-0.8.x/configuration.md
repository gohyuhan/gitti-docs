---
sidebar_position: 5
description: Gitti の設定 — 言語、初期化時のデフォルトブランチ、エディタ、コミットログ上限、コミットグラフ、マージ戦略、reflog 上限、ログ、署名 UI の挙動。
keywords:
  - gitti 設定
  - gitti config
  - gitti エディタ
...
ファイルの編集（例：ファイル上で`e`を押したとき）、または現在のリポジトリを開くとき（`O`を押したとき）に使用するエディタを設定できます。デフォルトのエディタは `vim` です。
...
## マージ戦略の設定 {#configure-merge-strategy}

ブランチをマージする際（ローカルブランチパネルの `m` から）、Gitti が fast-forward または non-fast-forward マージを使用するかどうかを制御できます。

**Fast-forward マージを有効にする**（`git merge --ff` を使用 — ブランチを fast-forward できる場合はマージコミットなし）：

```bash
gitti --ff-merge true
```

**Non-fast-forward マージを有効にする**（`git merge --no-ff` を使用 — 常にマージコミットを作成）：

```bash
gitti --ff-merge false
```

デフォルトは `false`（non-fast-forward）で、ブランチ統合の明確な履歴のために常にマージコミットを生成します。

## RefLog エントリ制限の設定

Gitti は RefLog パネルに設定可能な数の reflog エントリを読み込みます。この数は `--max-ref-log-count` フラグで調整できます。
