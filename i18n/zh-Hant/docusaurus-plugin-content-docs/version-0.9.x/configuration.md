---
sidebar_position: 5
description: 配置 Gitti — 語言、初始化預設分支、編輯器、提交日誌上限、提交圖、合併策略、reflog 上限、日誌與簽名 UI 行為。
keywords:
  - gitti 配置
  - gitti config
  - gitti 編輯器
...
您可以配置 Gitti 在打開檔案進行編輯時（例如：在檔案上按 `e`）或打開當前倉庫時（按 `O`）使用的編輯器。預設編輯器是 `vim`。
...
## 配置合併策略 {#configure-merge-strategy}
