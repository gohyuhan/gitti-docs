---
sidebar_position: 5
description: 配置 Gitti — 语言、初始化默认分支、编辑器、提交日志上限、提交图、合并策略、reflog 上限、日志与签名 UI 行为。
keywords:
  - gitti 配置
  - gitti config
  - gitti 编辑器
...
您可以配置 Gitti 在打开文件进行编辑时（例如：在文件上按 `e`）或打开当前仓库时（按 `O`）使用的编辑器。默认编辑器是 `vim`。
...
## 配置合并策略 {#configure-merge-strategy}

合并分支时（通过本地分支面板的 `m`），您可以控制 Gitti 使用 fast-forward 还是 non-fast-forward 合并。

**启用 fast-forward 合并**（使用 `git merge --ff` — 如果分支可以 fast-forward 则不产生合并提交）：

```bash
gitti --ff-merge true
```

**启用 non-fast-forward 合并**（使用 `git merge --no-ff` — 始终创建合并提交）：

```bash
gitti --ff-merge false
```

默认值为 `false`（non-fast-forward），始终生成合并提交以清晰记录分支集成历史。

## 配置 RefLog 条目限制

Gitti 在 RefLog 面板中加载可配置数量的 reflog 条目。您可以使用 `--max-ref-log-count` 标志调整此数量。
...
默认情况下，如果 Git 操作(如提交、标签或还原)可能需要您的 GPG 或 SSH 密码进行签名，Gitti 会挂起其 UI 并进入终端。如果您的密钥不需要手动输入，并且希望防止 Gitti 在这些操作期间挂起 UI，您可以覆盖此行为：
